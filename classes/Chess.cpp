...
std::string Chess::stateString()
{
    ...
    // std::string s;
    // for ( int x = 8; x < _gameOptions.rowX; x++){
    //     for (int y = 0; y < _gameOptions.rowY; y++){
    //         s += piece_notation(x, y)[1];
    //     }
    // }
    // return s;
...
void Chess::updateAI(){
    ...
    // cout << "Current state string: " << copyState << endl;
    for (auto move: _moves){
        memcpy(&baseState[0], copyState.c_str(), 64);
        ...
        int bestValue = -negamax(baseState, 3, 1, -myinfinity, myinfinity);
        // cout << "Best value from negamax for move: " << move.from << " to " << move.to << " is " << bestValue << endl;
